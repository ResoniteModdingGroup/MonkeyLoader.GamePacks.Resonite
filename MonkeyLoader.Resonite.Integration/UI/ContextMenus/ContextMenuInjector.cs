...
namespace MonkeyLoader.Resonite.UI.ContextMenus
{
    ...
    internal sealed class ContextMenuInjector : ResoniteAsyncEventSourceMonkey<ContextMenuInjector, ContextMenuItemsGenerationEvent>
    {
        ...
        private static bool InteractionHandlerOpenContextMenuPrefix(InteractionHandler __instance, MenuOptions options, float? speedOverride)
        {
            if (!Enabled)
                return true;
        ...
        private static async Task<ContextMenu?> ExtensionsOpenContextMenuPostfixAsync(Task<ContextMenu?> __result)
        {
